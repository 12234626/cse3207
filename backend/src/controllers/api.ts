...
async function getImageUrl(req: Request, res: Response) {
  try {
    ...
    fetch(`http://localhost:3000/db/image?id=${id}`)
      ...
      .then(function (data) {
        res.status(200).json(
          data.map(function (image: { path: string }) {
...
async function updateClub(req: Request, res: Response) {
  try {
    const { club_id, recruitment, introduction, info_post_id, content } =
      req.body;
    // const {path} = req.file as any;
  ...
  try {
    ...
    fetch(`http://localhost:3000/db/user${params ? "?" + params : ""}`)
      ...
      .then(async function (data) {
        res.status(200).json(
          await Promise.all(
            data.map(async function (user: { image_id: number }) {
              const image_url = (
                await (
                  await fetch(
                    `http://localhost:3000/api/image?id=${user.image_id}`
                  )
                ).json()
              )[0];
  ...
  try {
    ...
    const is_valid =
      (
        await (
          await fetch(
            `http://localhost:3000/db/user?id=${id}&password=${password_hash}`
          )
        ).json()
      ).length !== 0;
  ...
  try {
    ...
    fetch(`http://localhost:3000/db/post${params ? "?" + params : ""}`)
      ...
      .then(async function (data) {
        res.status(200).json(
          await Promise.all(
            data.map(async function (post: { image_id: number }) {
              const image_url = (
                await (
                  await fetch(
                    `http://localhost:3000/api/image?id=${post.image_id}`
                  )
                ).json()
              )[0];
...
async function createClubRequest(req: Request, res: Response) {
  try {
    ...
    fetch(
      `http://localhost:3000/db/club_request?club_id=${club_id}&user_id=${user_id}&status=대기`
    )
      .then(function (response) {
        return response.json();
      })
  ...
  try {
    ...
    if (status === "수락") {
      const { club_id, user_id } = (
        await (
          await fetch(`http://localhost:3000/db/club_request?id=${id}`)
        ).json()
      )[0];
    ...
    res.status(200).json({ message: "동아리 가입 신청 상태 업데이트 성공" });
  } catch (err) {
    res.status(500).json({ message: err });
  }
}
